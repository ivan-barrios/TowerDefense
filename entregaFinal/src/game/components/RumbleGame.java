...
import entregable.exceptions.DrawException;
...
public class RumbleGame {
	...
	public void nextRound() throws DrawException {
		...
		if ((playerOne.isCastleEmpty() && playerTwo.isCastleEmpty()) && (playerOne.getCastle().getCastleLife() >= 1)
				&& (playerTwo.getCastle().getCastleLife() >= 1)) {
			loopGame = false;
			throw new DrawException("Hubo un empate, se quedaron ambos sin monstruos y no perdio ninguno");
		}
	...
	public void startGame() { // Throws Exception
		...
		if (((playerOne.getCastle().getCastleLife() <= 0) || (playerTwo.getCastle().getCastleLife() <= 0))) {
			Resultado resultado = new Resultado(playerOne.getCastle().getCastleLife(),
					playerTwo.getCastle().getCastleLife());
